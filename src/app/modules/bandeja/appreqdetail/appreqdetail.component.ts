...
export class AppreqdetailComponent implements OnInit {
  ...
  actualizarReq(req: RequerimientoInterface): void {
    ...
    this._service
      .updateRequerimiento(req, date, this.usuario.user.userId)
      .subscribe((data) => {
        let message: string;
        this.getHistoricoCambios(this.reqId);
        this.getEstados();
        ...
        if (data.hasOwnProperty("errorMessage")) {
          ...
          if (
            this.requerimiento.estado !=
            this.historicoCambios[this.historicoCambios.length - 1].idEstado
          ) {
            //envio de correo si el estado es diferente al ultimo
            this._emailService.getEmailDataById("2").subscribe((data) => {
              ...
              if (data.hasOwnProperty("errorMessage")) {
                ...
                let estadoAntiguo: string =
                  this.historicoCambios[this.historicoCambios.length - 2]
                    .estado;
                let estadoNuevo: string =
                  this.historicoCambios[this.historicoCambios.length - 1]
                    .estado;

                let mailBody: string = this.replaceEmailValues(
                  dataEmail[0].body,
                  "@{NombrePET}",
                  this.requerimiento.nombre
                );
                let mailBody2: string = this.replaceEmailValues(
                  mailBody,
                  "@{Usuario}",
                  this.usuario.user.nombres +
                    " " +
                    this.usuario.user.apellidoPaterno
                );
                let mailBody3: string = this.replaceEmailValues(
                  mailBody2,
                  "@{EstadoAntiguo}",
                  estadoAntiguo
                );
                let mailBody4: string = this.replaceEmailValues(
                  mailBody3,
                  "@{EstadoNuevo}",
                  estadoNuevo
                );

                let mailSubject: string = this.replaceEmailValues(
                  dataEmail[0].subject,
                  "@{NombrePET}",
                  this.requerimiento.nombre
                );
                ...
                let toMail: string = this.replaceEmailValues(
                  dataEmail[0].toEmail,
                  "@{CorreoSolicitante}",
                  this.requerimiento.solicitanteCorreo
                );

                this._emailService
                  .sendEmail(mailSubject, mailBody4, toMail)
                  .subscribe((data) => {
                    let rest2: any = data;
  ...
  onFileSelected(files: FileList) {
    if (files.length > 0) {
      ...
      if (count == 0) {
        ...
        this._service
          .uploadFile(files[0], this.requerimiento.codigoSolicitud)
          .subscribe(
            (data) => {
              ...
              if (response.error == true) {
                ...
                let arc: any = data;
                let fecha2 = new Date();
                let dateF = this.datepipe.transform(
                  fecha2,
                  "yyyy/MM/dd HH:mm:ss"
                );
                ...
                this._service
                  .addFileLinkToDB(
                    parseInt(this.requerimiento.id),
                    arc.originalName,
                    arc.filePath,
                    dateF
                  )
