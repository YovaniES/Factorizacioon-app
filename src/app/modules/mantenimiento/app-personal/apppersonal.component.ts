...
import { PersonalClass, EntidadClass } from 'src/app/models/mantenimiento';
...
export class AppPersonalComponent implements OnInit {
  ...
  constructor(private _service: MantenimientoService, private spinner: NgxSpinnerService, 
  ...
  getListaRecursoSinPersonal(){//Para listar los recursos que no tiene el personal
    this.spinner.show();
    let arrayParametro:any[] = [{
      "queryId": 15
    }];
  ...
  quitarRecurso(idPersonal:number,idRecurso:number){
    ...
    for (let index = 0; index < this.recursoPersona.length; index++) {
      if (idRecurso != this.recursoPersona[index].idRecurso && idPersonal == this.recursoPersona[index].idPersonal) {
         //delete this.recurso[index]
  ...
  handlePageChange(event) {
    ...
    this.spinner.show();
      ...
      if(this.totalBusqueda != this.totalRequerimientos){
        this._service.getListaMantenimiento(offset.toString()).subscribe(data => {
          //this.personal = data;
          /*FRANCIA INICIO*/
          const arrayData:any[] = Array.of(data);
