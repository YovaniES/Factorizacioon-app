...
export class AppRecursoComponent implements OnInit {
  ...
  constructor(private _service: MantenimientoService, private spinner: NgxSpinnerService, 
    public datepipe: DatePipe, private toastr: ToastrService, private router: Router) { 
  ...
  buscarRecursoHardware(){
    ...
    if (this.serie == '' && this.idTipoBuscar == '' && this.idMarcaBuscar == '' && this.idEstadoBuscar == '' && this.imei == '') {
      this.getListaRecurso();
    }else{
      let arrayParametro:any[] = [{
        "queryId": 45,
