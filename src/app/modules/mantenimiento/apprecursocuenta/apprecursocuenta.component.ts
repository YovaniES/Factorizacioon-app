  ...
  getRecursoId(){
    ...
    this._service.getCuentaId(arrayParametro[0]).subscribe(data => {
      ...
      for (let index = 0; index < arrayData[0].list.length; index++) {
          ...
          this.datosCuenta.fechaUltimaRenovacion = arrayData[0].list[index].fecha_ultima_renovacion;
          this.datosCuenta.fechaProximaRenovacion = arrayData[0].list[index].fecha_proxima_renovacion;
        }
      this.spinner.hide();
    });
  }
