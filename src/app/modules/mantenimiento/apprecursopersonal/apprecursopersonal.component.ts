...
export class ApprecursopersonalComponent implements OnInit {
  ...
  getHistoricoProyectos(){
    ...
    this._service.getHistoricoPersonaProyectos(arrayParametro[0]).subscribe(data =>{
      this.spinner.hide();
      ...
      for(let index = 0; index < arrayData[0].list.length; index++){
        this.listaHistoricoProyectos.push({
          ...
          id_usuario_asignacion: arrayData[0].list[index].id_usuario_asignacion,
          nombre_usuario_asignacion: arrayData[0].list[index].nombre_usuario_asignacion
        });
      }
    });
  }
  ...
  getListaHardwareDisponible(){
    ...
    this._service.getListHardwareDisponible(arrayParametro[0]).subscribe(data => {
      const arrayData:any[] = Array.of(data);
  ...
  getListaCuentaDisponible(){
    ...
    this._service.getListCuentaDisponible(arrayParametro[0]).subscribe(data => {
      const arrayData:any[] = Array.of(data);
  ...
  getPersonalId(){
    ...
    this._service.getPersonalId(arrayParametro[0]).subscribe(data => {
      ...
      for (let index = 0; index < arrayData[0].list.length; index++) {
          ...
          if(arrayData[0].list[index].fecha_ingreso != 'null' && arrayData[0].list[index].fecha_ingreso != ''){
            let fechaI = arrayData[0].list[index].fecha_ingreso
            const str = fechaI.split('/');
            const year = Number(str[2]);
            const month = Number(str[1]);
            const date = Number(str[0]);
            this.datosPersonalEditar.fechaIngreso = this.datepipe.transform(new Date(year,month-1,date),'yyyy-MM-dd');
            console.log(this.datosPersonalEditar.fechaIngreso);
          }
          if(arrayData[0].list[index].fecha_nacimiento != 'null' && arrayData[0].list[index].fecha_nacimiento != ''){  
            ...
            this.datosPersonalEditar.fechaNacimiento = this.datepipe.transform(new Date(year2,month2-1,date2),'yyyy-MM-dd');
  ...
  actualizarFechaIngreso(fecha:string){
    this.datosPersonalEditar.fechaIngreso = this.datepipe.transform(fecha, 'yyyy-MM-dd');
  }

  actualizarFechaNacimiento(fecha:string){
    this.datosPersonalEditar.fechaNacimiento = this.datepipe.transform(fecha, 'yyyy-MM-dd');
  }
  ...
  buscarRecursoHardware(){
    ...
    if (this.serie == '' && this.idTipo == '' && this.idMarca == '') {
      ...
      this._service.buscarRecursoHardware(arrayParametro[0]).subscribe(data => {
        const arrayData:any[] = Array.of(data);
        this.listaHardwareDisponible = [];
        for (let index = 0; index < arrayData[0].list.length; index++) {
          this.listaHardwareDisponible.push({
