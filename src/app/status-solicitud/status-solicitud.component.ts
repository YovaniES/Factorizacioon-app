...
export class StatusSolicitudComponent implements OnInit {
  ...
  constructor(
    ...
    private _service: StatusRequerimientoService
  ) {}
  ...
  ngOnInit() {
    ...
    this._Activatedroute.paramMap.subscribe((params) => {
      ...
      this._service.getReqData(param1).subscribe(
        (result: any[]) => {
          ...
          this.desestimado = result.reduce(
            (acc, val) => (acc += val.Desestimado),
            0
          );
          this.cantidad = this.cantidad - this.desestimado;
          this.pendiente = result.reduce(
            (acc, val) => (acc += val.Pendiente),
            0
          );
          this.error = result.reduce((acc, val) => (acc += val.Errores), 0);
          this.bloqueado = result.reduce(
            (acc, val) => (acc += val.Bloqueado),
            0
          );
          this.avance =
            (100 * (this.cantidad - this.pendiente - this.bloqueado)) /
            this.cantidad;
          if (
            (100 *
              (this.cantidad - this.pendiente - this.bloqueado - this.error)) /
              (this.cantidad - this.pendiente - this.bloqueado) >
            0
          ) {
            this.passrate =
              (100 *
          ...
          /*DETALLE*/
          this._service.getCasosPruebaData(String(this.idProyecto)).subscribe(
            (result: any[]) => {
              this._spinner.hide();
              this.caspru = result;
          ...
          /*GET HALLAZGOS*/
          this._service.getHallazgoData(String(this.idProyecto)).subscribe(
            (result: any[]) => {
              ...
              var difficult_tasks = result.filter(
                (result) => result.estado == "Cerrada"
              );
              ...
              var difficult_tasks2 = result.filter(
                (result) => result.estado == "Cancelado"
              );
