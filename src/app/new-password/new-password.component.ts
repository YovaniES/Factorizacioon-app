...
export class NewPasswordComponent implements OnInit {
  ...
  changePassword() {
    if (this.newPassword != this.confirmPassword) {
      ...
      this._spinner.show();
      this._resetPassword
        .getUsuarioIdByToken(this.uniqueId)
        .subscribe((data) => {
          if (data.hasOwnProperty("errorMessage") || data == "undefined") {
            ...
            const md5 = new Md5();
            let hashnewPassword: string = md5
              .appendStr(this.newPassword)
              .end()
              .toString();
            this._resetPassword
              .changePassword(userData[0].userId, hashnewPassword)
              .subscribe((data) => {
                this._spinner.hide();
                this._resetPassword
                  .deleteResetRequest(this.uniqueId)
                  .subscribe((data2) => {
                    console.log(data);
                  });
                Swal.fire({
