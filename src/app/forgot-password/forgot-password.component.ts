...
export class ForgotPasswordComponent implements OnInit {
  ...
  constructor(private _resetPassword: ChangePasswordService, private _mail: EmailService, 
        ...
        if(data[0].hasOwnProperty('userId')){
          ...
          this._resetPassword.getEmailToken(this.email).subscribe(data => {
            ...
            if(data.hasOwnProperty('token')){
              let result:any = data;
              console.log(result);
              this._resetPassword.saveResetLink(result.token, usuarioData[0].userId).subscribe(data => {
                this._mail.getEmailDataById('3').subscribe(data => {
                  ...
                  if(data.hasOwnProperty('errorMessage')){
                    ...
                    let mailBody:string = this.replaceEmailValues(dataEmail[0].body,"@{Link}", 'http://dynamob2b.indratools.net/newPassword/'+result.token);
        
                    let toMail:string = this.replaceEmailValues(dataEmail[0].toEmail, "@{CorreoIngresado}", this.email);
        
                    this._mail.sendEmail(dataEmail[0].subject, mailBody, toMail).subscribe(data => {
                      this.spinner.hide();
                      let rest2:any = data;
